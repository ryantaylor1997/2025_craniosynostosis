...
\begin{aligned}
\bS^{(b)} &= \begin{bmatrix}
\bS_b & \bzero_{k_b \times k_f} \\
\bzero_{k_f \times k_b} & \bzero_{k_f \times k_f}
\end{bmatrix} \\
\bS^{(f)} &= \begin{bmatrix}
\bzero_{k_b \times k_b} & \bzero_{k_b \times k_f} \\
\bzero_{k_f \times k_b} & \bS_f
\end{bmatrix}
\end{aligned}
$$
...
\begin{bmatrix}
\lambda_b \bS_b & \bzero_{k_b \times k_f} \\
\bzero_{k_f \times k_b} & \lambda_f \bS_f
\end{bmatrix}.
$$
...
\bS^{(\lambda) \ -}_T =
\begin{bmatrix}
\frac{1}{\lambda_b} \bS_b^- & \bzero_{k_b \times k_f} \\
\bzero_{k_f \times k_b} & \frac{1}{\lambda_f} \bS_f^-
\end{bmatrix}.
$$
...
$$
\bbeta = \begin{pmatrix} \bbeta_{b \ [k_b \times 1]} \\ \bbeta_{f \ [k_f \times 1]} \end{pmatrix},
$$
where $\bbeta_b$ are the coefficients corresponding to the boundary spline and $\bbeta_f$ are the coefficients corresponding to the interior spline.
...
\bgamma_i & \sim \mathrm{N}_K(\bzero, \sigma^2 \bS^{(\lambda) \ -}_T) \\
...
$$
\begin{aligned} 
\ [\lambda_b | \bbeta_b, \sigma^2] & \sim \mathrm{Gamma} \Bigg( \frac{k_b}{2} + c_b, \ \frac{1}{2 \sigma^2} \bbeta_b^\top S_b \bbeta_b + d_b \Bigg) \\
[\lambda_f | \bbeta_f, \sigma^2] & \sim \mathrm{Gamma} \Bigg( \frac{k_f}{2} + c_f, \ \frac{1}{2 \sigma^2} \bbeta_f^\top S_f \bbeta_f + d_f \Bigg) \\
[\sigma^2 | \bbeta, \lambda_b, \lambda_f] & \sim \mathrm{Inv. Gamma} \Bigg( \frac{m}{2} + \frac{K}{2} + a, \ \frac{1}{2} \Big[ \bG^\top \bG  - 2 \bbeta^\top \bC^\top \bG + \bbeta^\top \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big) \bbeta \Big] + b \Bigg) \\
[\bbeta | \sigma^2, \lambda_b, \lambda_f] & \sim \mathrm{N}_K \Bigg( \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big)^{-1} \bC^\top \bG, \ \sigma^2 \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big)^{-1} \Bigg)
\end{aligned}
$$
...
Gibbs.Pen.Lin.Reg_FN <- function(
  ...
  for(iter in 1:iters){
    ...
    b_post <- 
      b_pri - t(y) %*% X %*% beta + 
      0.5 * (sum(y^2) + t(beta) %*% xtx_plus_pen %*% beta)
    ...
    d_post <- map(S,
                  function(S_j){
                    (1 / (2 * sigmasq)) * t(beta) %*% S_j %*% beta + d_pri
                  })
