...
-   Bayesian soap film smoother was fit on one individual patient (@sec-bayes-individual)
-   Assumptions for Bayesian model were written out (@sec-bayes-soap)
...
From this smoother object, we extract the relevant matrices that will allow us to fit the model: the $n \times K$ design matrix $X$ and the penalty matrices for the boundary and interior splines ($\mathbf{S}^{(1)}$ and $\mathbf{S}^{(2)}$ respectively). In `mgcv`, The smoother object stores each penalty matrix as size $K \times K$. So we can denote , even though $K = k_1 + k_2$, where $k_1$ is the dimension of the boundary spline and $k_2$ is the dimension of the interior film spline. To see the effect of each of these penalty matrices, we extract each individual matrix excluding the block 0 matrices.

We estimate the coefficients for this model using the closed-form expression
$$
\hat{\boldsymbol{\beta}} = (\mathbf{X}^\top \mathbf{X} + \boldsymbol{\lambda} \mathbf{S^*})^{-1}\mathbf{X}^\top \mathbf{y}
$$
...
# Plot model with lowest GCV for each patient
plot_so_map_nobound <- ggplot(film_map_optim_fit) +
  geom_raster(aes(x = row, y = col, fill = fit)) +
  scale_fill_viridis_c(option = "turbo", limits = so_diff_range) +
  labs(x = NULL, y = NULL, 
       fill = "Pred. Growth exc. Bound.",
...
# Combine and print
map_film_both <- ggarrange(plot_so_map_nobound, 
                           plot_so_test_data, 
                           ncol = 1)
map_film_both
      ...
      function(d){
        ...
        fits <- so_constants %>% 
          ...
          mutate(fit = map(coeffs, ~ so_X %*% .x),
                 ...
                 fit_int = map(coeffs, ~ so_X[, film_cols] %*% .x[film_cols])) %>% 
...
# Plot only boundary spline from model with lowest GCV for each patient
plot_map_soap_bound <- ggplot(so_test_fit) +
  geom_raster(aes(x = row, y = col, fill = fit_bound)) +
  scale_fill_viridis_c(option = "turbo", limits = so_diff_range) +
  labs(x = NULL, y = NULL, fill = "Pred. Growth", 
       title = "Predictions: Boundary Only") +
  coord_fixed() +
  theme_void() +
  theme(legend.position = "bottom") +
  facet_wrap(~fusion_type, nrow = 1)
...
# Compare predictions from interior, boundary, both, and truth
map_soap_build <- ggarrange(
  plot_so_map_nobound,
  plot_map_soap_bound,
  plot_map_soap,
  plot_so_test_data,
...
Gibbs.Pen.Lin.Reg_FN <- function(
  ...
  for(j in 1:iters){
    ...
    if(j %% iters_burn == 0) cat("at turn", j, "\n")
    ...
    beta_helper <- solve( t(X) %*% X + lambda_S )
    ...
    b_post <- 
      0.5 * t((y - X %*% beta)) %*% (y - X %*% beta) + 
      0.5 * t(beta) %*% lambda_S %*% beta + 
      b_pri
    ...
    if(iter > 0){
      beta_out[iter,] <- beta
      sigmasq_out[iter,] <- sigmasq
      lambda_out[iter,] <- lambda
    } else{
      beta_burn[j,] <- beta
      sigmasq_burn[j,] <- sigmasq
      lambda_burn[j,] <- lambda
    }
  }
...
# Save time length
soap_bayes_time <- difftime(Sys.time(), bayes_soap_start, units = "mins")
