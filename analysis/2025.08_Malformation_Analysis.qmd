...
sigma_trace <- ggplot(soap_bayes1_tr %>% 
  ...
  facet_grid(name ~ draw_cat, scales = "free")
...
setup_reference_model <- function(sm, drop_col = NULL, by_var, newdata = NULL, 
  ...
  if(is.null(param_formula)){
    ...
    S_unpenal <- matrix(0, ncol(design_param), ncol(design_param))
  }
  
  # Merge with spline matrix for full design matrix
  X_design <- cbind(design_param, ref_splines$X)
  
  S_full <- map(ref_splines$S, ~as.matrix(bdiag(S_unpenal, .x)))
  
  out <- list("X" = X_design)
  
  if(is.null(newdata)){ out[["S"]] <- S_full }
...
Let $\bG \ (N \times M)$ represent the matrix of growth measurements at locations $s_1, ..., s_M$ for individuals $1, ..., N$. Therefore, let $G_i(s_m)$ represent the growth at location $s_m$ for individual $i$. 

Let $\bB(s_m)$ be the $K \times 1$ vector of soap film basis functions evaluated at location $s_m$, and let $\bgamma_i$ be the coefficients multiplied by these soap film basis functions.

We assume:
$$
\begin{aligned}
G_i(s_m) &\sim N(\bgamma \bB(s_m), \sigma^2) \\
\implies \bG_i(\bs)^\top & \sim N_M(\bB(\bs) \bgamma_i, \sigma^2 \bI_M)
\end{aligned}
$$

Next, we assume the prior:

$$
\bgamma_i \sim N_K(\bbeta^\top \bX_i, \sigma^2 \bP_\lambda^{(\gamma) \ -})
$$
where:

$\bX_i$ is a $1 \times Q$ row vector of patient-level fixed effect covariates. This matrix includes some parametric fixed effects and some penalized covariates whose effects are estimated with $R$ different GAMs. So we can also represent $\bX_i$ as:
$$
\bX_i = \begin{bmatrix}
\bw^{(u)}_i & \bb_0(\bw^{(p)}_i) & \ldots & \bb_{R-1}(\bw^{(p)}_i)
\end{bmatrix}
$$

where $\bw^{(u)}_i$ is a $1 \times Q_u$ row vector of unpenalized covariates, $\bw^{(p)}_i$ is a $1 \times Q_p$ row vector of penalized covariates, and $\bb_r(\cdot)$ is a $1 \times L$ row vector of basis functions of $\bw^{(p)}$.

Then $\bbeta$ is a $Q \times K$ matrix of coefficients describing the effects of the covariates described above on the soap film basis functions $\bgamma_i \bB$; and

$$\bP_\lambda^{(\gamma)} = \begin{bmatrix}
\lambda_b \bP_b & \bzero \\
\bzero & \lambda_f \bP_f
\end{bmatrix}$$

is a penalty matrix encouraging smoothness of spatial functions across locations $\bs$. Then $\bP_\lambda^{(\gamma) \ -}$ is the generalized inverse of this penalty matrix.

This prior assumption on $\bgamma_i$ yields the posterior likelihood:

$$
[\bgamma_i | \bG_i, \sigma^2, \bbeta] \sim N_K \Bigg( \Big(\bB^\top \bB + \bP_\lambda^{(\gamma)} \Big)^{-1} \Big( \bB^\top \bG_i + \bP_\lambda^{(\gamma)} \bbeta^\top \bX_i \Big), \ \sigma^2 \Big(\bB^\top \bB + \bP_\lambda^{(\gamma)} \Big)^{-1} \Bigg)
$$
