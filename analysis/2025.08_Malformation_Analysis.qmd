...
-   Written regression equations now use FDA notation (@sec-prelim-models)
-   Patients above the age of 5 were removed (@sec-age-summary)
-   Preliminary fusion-specific GAM plots are wider and now include an outline to show area within the intracranial mask (@sec-prelim-gam)
-   A soap film smoother object was created based on the data (@sec-soap-film-grid)
-   Models were fit on individual patients using matrix multiplication: first with a loop on a subset (@sec-soap-film-loop), then using nested tibbles with a subset (@sec-soap-film-map), and finally on the entire data (@sec-soap-film-individual)
...
# Clear memory space
rm(list = setdiff(ls(), ls(pattern = eval(RM_IGNORE))))
...
cranio_soap <- smoothCon(
  s(row, col, bs = "so", xt = list(bnd = bound_list)),
...
for(i in 1:nrow(so_test_df)){
  for(j in 1:nrow(so_constants)){
    ...
    soap_loop_df %<>% bind_rows(out_dl)
  }}

# Plot GCV from test models
soap_loop_gcv_plot <- ggplot(soap_loop_df) +
  geom_raster(aes(x = sp_bound, y = sp_film, fill = gcv)) +
  facet_wrap(~fname, scales = "free_y") +
  scale_x_continuous(transform = "log10") +
...
# Take model with lowest GCV for each patient
soap_loop_optim <- soap_loop_df %>% 
  arrange(fname, gcv, desc(sp_film), desc(sp_bound)) %>% 
  group_by(fname) %>% 
  slice(1) %>% 
  ungroup() %>% 
  left_join(so_test_df, by = "fname")
...
# Extract data and fitted values for plotting
soap_loop_optim_fit <- soap_loop_optim %>% 
  select(fname, fusion_type, fit, data) %>% 
  unnest(c(fit, data))

# Keep predicted and actual plots consistent
soap_loop_range <- soap_loop_optim_fit %$%
  c(
  min(c(min(diff), min(fit))),
  max(c(max(diff), max(fit)))
)
...
# Plot model with lowest GCV for each patient
plot_loop_soap <- ggplot(soap_loop_optim_fit) +
  geom_raster(aes(x = row, y = col, fill = fit)) +
...
# Plot actual observations
plot_loop_soap_data <- ggplot(soap_loop_optim_fit) +
  geom_raster(aes(x = row, y = col, fill = diff)) +
  scale_fill_viridis_c(option = "turbo", limits = soap_loop_range) +
  labs(x = NULL, y = NULL, fill = "Actual Growth") +
  coord_fixed() +
...
soap_map_df <- so_test_df %>%
  # Expand data to one row per patient and penalty constant
  mutate(constants = list(so_constants %>% select(-visor))) %>%
  unnest(constants) %>% 
  # Estimate coefficients
  mutate(coeffs = map2(constant_row, data,
                       ~so_constants$visor[[.x]] %*% .y$diff)) %>% 
  # Fitted values
...
# Extract data and fitted values for plotting
soap_map_optim_fit <- soap_map_optim %>% 
  select(fname, fusion_type, fit, data) %>% 
  unnest(c(fit, data))

# Keep predicted and actual plots consistent
soap_map_range <- soap_map_optim_fit %$%
  c(
  min(c(min(diff), min(fit))),
  max(c(max(diff), max(fit)))
)
...
# Plot model with lowest GCV for each patient
plot_map_soap <- ggplot(soap_map_optim_fit) +
  geom_raster(aes(x = row, y = col, fill = fit)) +
...
# Plot actual observations
plot_map_soap_data <- ggplot(soap_map_optim_fit) +
  geom_raster(aes(x = row, y = col, fill = diff)) +
  scale_fill_viridis_c(option = "turbo", limits = soap_map_range) +
  labs(x = NULL, y = NULL, fill = "Actual Growth") +
  coord_fixed() +
...
cranio_model_optim <- cranio_models %>% 
  arrange(fname, gcv, desc(sp_film), desc(sp_bound)) %>% 
  group_by(fname) %>% 
  slice(1) %>% 
  ungroup()
...
cranio_model_df <- cranio_model_optim %>% 
  select(fname, coeffs) %>% 
  mutate(coeffs = map(coeffs, as.vector)) %>% 
  rename(beta_film = coeffs) %>% 
  left_join(cranio_sub, by = "fname") %>% 
  left_join(fusion_dict, by = "fusion_type") %>% 
  select(everything(), beta_film)
  unnest_wider(beta_film, names_sep = "_", simplify = T)

write_csv(cranio_model_df, 
          here::here("intermediate", "individual_interior_coeffs.csv"))
