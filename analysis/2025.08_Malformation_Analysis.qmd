...
-   Added plots to visualize the individual soap film basis functions (@sec-soap-basis-fns)
-   Wrote a C++ version of the beta update, then updated it to modify beta moments in place
-   Added timing of the C++ beta update in the MCMC loop (@sec-mcmc-diagnostics)

## Questions

-   How much of the MCMC will be moved to Cpp?
...
Gibbs.Pen.Lin.Reg_FN <- function(
  ...
  for(iter in 1:iters){
    ...
    # Save current versions so we can check if functions get updated
    xtx_plus_pen_old <- as.vector(xtx_plus_pen)
    
    beta_expect_old <- as.vector(beta_expect)
    
    beta_var_old <- as.vector(beta_var)
    ...
    # Save current versions so we can compare R vs. Cpp
    xtx_plus_pen_C <- as.vector(xtx_plus_pen)
    
    beta_expect_C <- as.vector(beta_expect)
    
    beta_var_C <- as.vector(beta_var)
    ...
    beta_expect <- tcrossprod(beta_helper, X) %*% y
    ...
    cat("xtx_pen in-place max diff:", max(abs(xtx_plus_pen_C - xtx_plus_pen_old)), "\n")
    cat("expect in-place max diff:", max(abs(beta_expect_C - beta_expect_old)), "\n")
    cat("var in-place max diff:", max(abs(beta_var_C - beta_var_old)), "\n")
    
    cat("xtx_pen R-C max diff:", max(abs(as.vector(xtx_plus_pen) - xtx_plus_pen_C)), "\n")
    cat("expect R-C max diff:", max(abs(as.vector(beta_expect) - beta_expect_C)), "\n")
    cat("var R-C max diff:", max(abs(as.vector(beta_var) - beta_var_C)), "\n")
    
    # Generate new beta sample
    beta <- rmvnorm(n = 1, beta_expect, beta_var) %>% t()
