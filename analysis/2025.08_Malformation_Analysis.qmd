...
-   Fit Bayesian soap film smoother on one individual patient (@sec-bayes-individual)
-   Wrote out assumptions for Bayesian model (@sec-bayes-soap)
-   Plotted posterior mean predictions from this Bayesian model (@sec-bayes-soap-plot)
-   Made trace plots of posterior and burn-in period, which show relatively fast convergence of the parameters (@sec-mcmc-diagnostics)
-   Added timing to track how long each part of the Gibbs sampler takes (@sec-mcmc-diagnostics)
-   Changed calculation for $\sigma^2$ in the Gibbs sampler to try to speed up that step *(not sure this worked)* (@sec-bayes-individual)
-   Changed notation changed for Bayesian models and earlier soap film descriptions to clear the way for multiple individuals / hierarchical data (@sec-soap-film-components)
- Compared plots of (GCV-optimized) predictions based on only interior spline, only boundary spline, and both (@sec-soap-map); investigated the boundary splines

## Questions

-   Is my notation consistent with the grant?
-   Should variable names in my code be consistent with notation? (e.g. right now I'm using X in the code for the design matrix but C in notation).
...
It is useful to visualize these basis functions to understand the effect each has on predictions.
...
$$
\hatbgamma_i = (\bC^\top \bC + \bS^{(\lambda)}_T)^{-1}\bC^\top \bG
$$

We run a grid search to find the optimal smoothing penalty constants $\lambda_1$ and $\lambda_2$ (called "sp" in mgcv). We use the range $10^{-9}, ..., 10^{-2}$. We then select the model that achieves the minimum generalized cross-validation ("GCV") score:
$$
\begin{gather*}
...
For this subset, we make predictions using only the interior spline, only the boundary spline, and both, and compare all three to the actual data. This shows an interesting result: when we exclude the boundary spline entirely and estimate coefficients, the resulting predicted values are almost all non-negative. When we estimate the coefficients using both splines and then model their contributions separately, the contribution from the interior is often negative, resulting in predictions near 0. This is because the basis functions in the design matrix are not just defined on the boundary; they are defined over the entire space, based on the boundary condition described by a cyclic boudnary spline. This is described in section 4.1 in the soap film paper (p. 940):

First, suppose values on the boundary follow a function $\bG_i^{(B)} = f_b(r)$, where $r$ is the distance along the boundary loop from an arbitrary starting point. 

Second, represent that function with the basis expansion $f_b(r) = \sum_{j=1}^J \alpha_j \delta_j(r)$. To estimate $\balpha$, include a penalty functional $\balpha^\top \bS_b \balpha$.

Third, for each $j = 1, \ldots, J$, determine the basis-specific boundary condition by setting $\alpha_j = 1$ and $\alpha_l = 0, \ l \neq j$. *I think this means all we're left with is * $f_b(r) = \delta_j(r)$. Solve for the function $a_j(x,y)$ which satisfies this boundary condition based on $\delta_j$ and satisfies the equation $\frac{\partial^2 a_j}{\partial x^2} + \frac{\partial^2 a_j}{\partial y^2} = 0$. 

Then the ``boundary-condition-induced-function" $a(x,y)$ has the basis function representation $a(x,y) = \sum_{j=1}^J \alpha_j a_j(x,y)$.
...
$$
\bgamma = \begin{pmatrix} \smash{\overbrace{\bgamma_{b}}^{k_b \times 1}} \\ \smash{\underbrace{\bgamma_{f}}_{k_f \times 1}} \end{pmatrix},
$$
where $\bgamma_b$ are the coefficients corresponding to the boundary spline and $\bgamma_f$ are the coefficients corresponding to the interior spline.

Then we assume the (conjugate) priors:

$$
\begin{aligned}
\bG_i(x,y) & \sim \mathrm{N}_m ( \bC^\top \bgamma_i, \sigma^2 \bI_m) \\
\bgamma_i & \sim \mathrm{N}_K \Big( \bzero, \sigma^2 \bS^{(\lambda) \ -}_T \Big) \\
\sigma^2 & \sim \mathrm{Inv. Gamma} (a, b) \\
\lambda_b & \sim \mathrm{Gamma} (c_b, d_b) \\
\lambda_f & \sim \mathrm{Gamma} (c_f, d_f) \\
\lambda_b & \perp \lambda_f
\end{aligned}
$$

We choose simple priors: $a = 1, \ b = 1,  \ c_b = c_f = 1, d_b = d_f = 1$. We have not yet checked how informative they are.

These priors lead to the full conditionals:

$$
\begin{aligned} 
\ [\lambda_b | \bgamma_b, \sigma^2] & \sim \mathrm{Gamma} \Bigg( \frac{k_b}{2} + c_b, \ \frac{1}{2 \sigma^2} \bgamma_b^\top S_b \bgamma_b + d_b \Bigg) \\
[\lambda_f | \bgamma_f, \sigma^2] & \sim \mathrm{Gamma} \Bigg( \frac{k_f}{2} + c_f, \ \frac{1}{2 \sigma^2} \bgamma_f^\top S_f \bgamma_f + d_f \Bigg) \\
[\sigma^2 | \bgamma, \lambda_b, \lambda_f] & \sim \mathrm{Inv. Gamma} \Bigg( \frac{m}{2} + \frac{K}{2} + a, \ \frac{1}{2} \Big[ \bG^\top \bG  - 2 \bgamma^\top \bC^\top \bG + \bgamma^\top \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big) \bgamma \Big] + b \Bigg) \\
[\bgamma | \sigma^2, \lambda_b, \lambda_f] & \sim \mathrm{N}_K \Bigg( \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big)^{-1} \bC^\top \bG, \ \sigma^2 \Big( \bC^\top \bC + \bS^{(\lambda)}_T \Big)^{-1} \Bigg)
\end{aligned}
$$
...
Gibbs.Pen.Lin.Reg_FN <- function(
    ...
    y, X, # Data
    S, # Penalty matrices
    sigmasq0, lambda0, # Default parameters
  ...
  for(iter in 1:iters){
    ...
    beta_updated <- penalized_beta_update(
      y = y, X = X, lambda_S = lambda_S, sigmasq = sigmasq
    )
    
    xtx_plus_pen_C <- beta_updated[["xtx_pen"]]
    
    beta_ex_C <- beta_updated[["coeff_mean"]]
    
    beta_var_C <- beta_updated[["coeff_var"]]
    ...
    xtx_plus_pen <- crossprod(X) + lambda_S
    beta_helper <- solve(xtx_plus_pen)
    
    timer["x_inverse"] <- timer["x_inverse"] + 
      difftime(Sys.time(), time_check, units = "secs")
    time_check <- Sys.time()
    
    # Posterior conditional expectation
    beta_ex <- tcrossprod(beta_helper, X) %*% y
    
    # Posterior conditional variance
    beta_var <- sigmasq * beta_helper
    
    timer["beta_moments"] <- timer["beta_moments"] + 
      difftime(Sys.time(), time_check, units = "secs")
    time_check <- Sys.time()
    
    print(sum(xtx_plus_pen - xtx_plus_pen_C))
    print(sum(beta_ex - beta_ex_C))
    print(sum(beta_var - beta_var_C))
...
# 
# # Output time
# cat(bayes_soap_iters, "MCMC iterations time:", so_bayes_time, "mins")
```
